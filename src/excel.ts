...
function normalizeHeader(h: string): string {
  ...
  if (["particulars", "name", "company", "stock"].includes(s))
    return "particulars";
  if (
    ["purchaseprice", "buyprice", "price", "avgprice", "averageprice"].includes(
      s
    )
  )
    return "purchasePrice";
  if (["qty", "quantity", "shares"].includes(s)) return "qty";
  if (["nse", "bse", "nsebse", "exchange", "symbol", "ticker"].includes(s))
    return "nseBse";
  if (["sector", "industry"].includes(s)) return "sector";
...
export function readExcel(): ExcelRow[] {
  ...
  const header = (rows[headerRowIndex === -1 ? 0 : headerRowIndex] || []).map(
    (h) => normalizeHeader(String(h))
  );
  ...
  for (
    let i = headerRowIndex === -1 ? 1 : headerRowIndex + 1;
    i < rows.length;
    i++
  ) {
    const row = rows[i];

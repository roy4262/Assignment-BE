...
export async function enrichData(rows: ExcelRow[]): Promise<StockData[]> {
  const totalInvestment = rows.reduce(
    (sum, r) => sum + r["Purchase Price"] * r.Qty,
    0
  );
  ...
  const result = await processInBatches(
    ...
    async (r): Promise<StockData> => {
      ...
      const yahooTicker =
        (await resolveToYahooTicker(raw, r.Particulars)) || raw;

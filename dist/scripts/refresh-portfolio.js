...
const INPUT_PATH = process.env.PORTFOLIO_JSON_IN ||
    path.resolve(process.cwd(), "portfolio.json");
const OUTPUT_PATH = process.env.PORTFOLIO_JSON_OUT ||
...
async function getExchangeSymbols(exchange) {
    ...
    const url = `https://finnhub.io/api/v1/stock/symbol?exchange=${encodeURIComponent(exchange)}&token=${CONFIG.FINNHUB_KEY}`;
    const res = await axios.get(url);
    return res.data;
}
async function searchYahooSymbolByName(name) {
    try {
        ...
        const nse = quotes.find((q) => typeof q?.symbol === "string" && q.symbol.endsWith(".NS"));
        if (nse?.symbol)
            return nse.symbol;
...
async function resolveSymbols(portfolio, symbols) {
    ...
    for (const holding of portfolio) {
        ...
        if (result.length > 0) {
            ...
            if (first && first.item) {
                ...
                const baseCandidate = !isNumeric(rawSymbol) && rawSymbol
                    ? rawSymbol
                    : isNumeric(disp)
                        ? ""
                        : disp;
...
async function enrichWithYahoo(portfolio) {
...
async function main() {
    ...
    const holdings = Array.isArray(raw?.holdings) ? raw.holdings : raw;
...
//# sourceMappingURL=refresh-portfolio.js.map
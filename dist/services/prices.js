...
export async function resolveToYahooTicker(raw, hintName) {
    ...
    try {
        ...
        const search = await yahooFinance.search(term, {
            quotesCount: 10,
            newsCount: 0,
        });
        ...
        const preferNse = (q) => {
            const exch = String(q.exchange || q.exch || q.exchDisp || "").toUpperCase();
        ...
        const nseQuote = equities.find(preferNse) || equities.find((q) => /\.NS$/i.test(q.symbol));
        if (nseQuote?.symbol && (await tryYahoo(nseQuote.symbol))) {
            cache.set(cacheKey, nseQuote.symbol, RESOLVE_TTL_MS);
            return nseQuote.symbol;
        }
        // 2b) else BSE symbol
        const bseQuote = equities.find(preferBse) || equities.find((q) => /\.BO$/i.test(q.symbol));
    ...
    try {
        const term = hintName?.trim() || code;
        const restUrl = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(term)}&lang=en-IN&region=IN&quotesCount=10&newsCount=0`;
        const r = await fetch(restUrl);
        const j = (await r.json());
        const quotes = Array.isArray(j?.quotes) ? j.quotes : [];
        const pick = (suffix) => quotes.find((q) => typeof q?.symbol === "string" && q.symbol.endsWith(suffix))?.symbol;
        const candNS = pick(".NS") ||
            quotes.find((q) => /\.NS$/i.test(q?.symbol))?.symbol;
        if (candNS && (await tryYahoo(candNS))) {
            cache.set(cacheKey, candNS, RESOLVE_TTL_MS);
            return candNS;
        }
        const candBO = pick(".BO") ||
            quotes.find((q) => /\.BO$/i.test(q?.symbol))?.symbol;
    ...
    if (CONFIG.ALPHAVANTAGE_KEY) {
        try {
            const url = `https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${encodeURIComponent(hintName?.trim() || code)}&apikey=${CONFIG.ALPHAVANTAGE_KEY}`;
            const res = await fetch(url);
            const json = (await res.json());
            const best = Array.isArray(json?.bestMatches)
                ? json.bestMatches[0]
                : null;
            const bestSymbol = best?.["1. symbol"];
            if (bestSymbol) {
...
export async function getCMP(rawSymbol) {
...
export async function getCurrentMarketPrice(ticker) {
    const q = await yahooFinance.quote(ticker);
    const price = Number(q?.regularMarketPrice ?? 0);
